---
name: cst-use
description: Create, modify, run, diagnose, and verify native electromagnetic simulation projects in CST Studio Suite, especially CST 2024 Microwave Studio. Use when Codex must operate CST, build editable antennas/waveguides/metasurfaces/RIS or periodic unit cells, configure materials/boundaries/ports/Floquet modes/solvers, preserve editable History Tree primitives and Boolean operations, reproduce a paper model, extract S-parameters or phases, troubleshoot CST licenses and crashes, or archive solved `.cst` projects and result folders.
---
...
5. When the user requires editable solids, create exactly one primitive per History node using CST-native captions and commands; never group several `Brick.Create` calls into one generic history block. Read [references/native-editable-history.md](references/native-editable-history.md) and perform its UI acceptance test.
6. Resolve positive-volume overlap between same-material solids with separate, named Boolean history nodes. Do not union merely touching layers or intentionally distinct materials. Verify consumed tool solids disappear from the final solid tree while the primitive and Boolean history remain traceable.
7. Work from a short ASCII-only directory when CST cannot save under a non-ASCII Windows profile. After solving, copy both the `.cst` file and its same-name result directory to the user-designated folder, excluding `*.lok` files.
8. Save before solving, run `model3d.run_solver()`, save again, then inspect the result tree through `cst.results.ProjectFile`.
9. Do not claim success merely because CST returned. Require the expected result items and numeric samples. For multi-state models, verify trends and key frequencies, not just screenshots.
10. Leave the final solved project easy to inspect in CST when practical. Deliver the native project, associated result directory, exported CSV, assumptions, and a concise verification report.
...
- Each requested editable primitive exposes a selectable `Define brick` or corresponding native primitive History child, and its editor opens with the expected parameter expressions.
- Same-material positive-volume overlaps are removed by traceable Boolean operations; no consumed tool solid remains in the final solid tree.
...
- Read [references/native-editable-history.md](references/native-editable-history.md) whenever solids must remain editable, History Tree shows `Not editable history block`, or overlapping solids require Boolean cleanup.
